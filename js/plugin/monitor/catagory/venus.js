define(['/js/plugin/monitor/monitorInfo.js',
	...
	return {
		...
		cpu_use_chart : function (el ,paramObj,type){
            var url="";
            if(type=="cpu"){
                url="monitorview/security/venusids/queryVenusIdsCpuInfo";
            }else{
                url="monitorview/security/venusids/queryVenusIdsMemoryInfo";
            }
			um_ajax_get({
		        url : url,
		        paramObj : paramObj,
		        ...
		        successCallBack : function (data){
		            ...
		            if (type == "memory")
		            {
			            ...
			            for (var i = 0; i < memStore.length; i++) {
                            categoryArray.push(cpuStore[i].updateTime);
